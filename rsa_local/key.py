...
def calculate_keys_custom_exponent(p, q, exponent):
    ...
    """
    import random
    exponent = random.randint(1,655370)
    phi_n = (p - 1) * (q - 1)
...
def gen_keys_pq(nbits, getprime_func, p, q, accurate=True, exponent=DEFAULT_EXPONENT):
    """Generate RSA keys of nbits bits. Returns (p, q, e, d).
...
def newkeys(nbits, pq = None, accurate=True, poolsize=1, exponent=DEFAULT_EXPONENT):
    ...
    if pq is None:
        (p, q, e1, d1) = gen_keys(nbits, getprime_func, accurate=accurate, exponent=exponent)
    else:
        p, q = pq
        (p, q, e1, d1) = gen_keys_pq(nbits, getprime_func, p, q, accurate=accurate, exponent=exponent)
